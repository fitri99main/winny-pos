...
async function run() {
  ...
  for (const s of filtered) {
    if (s.status !== 'Returned') {
       if (!branches[s.branch_id]) branches[s.branch_id] = { total: 0, count: 0 };
       branches[s.branch_id].total += s.total_amount;
       branches[s.branch_id].count++;
    }
  }
  
  console.log(`Per Branch 30 Days:`, branches);
}
run();
