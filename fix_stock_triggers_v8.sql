-- ==========================================
-- FIX STOCK TRIGGERS (V8 - RESTRICTED STATUS)
-- Only deducts stock for 'Completed' or 'Selesai'
-- Prevents deduction for 'Paid' or 'Bayar'
-- ==========================================

-- 1. UNIFIED STOCK HANDLER FUNCTION (V8)
...
BEGIN
    ...
    IF v_is_delete THEN
        ...
        -- V8: Removed 'paid', 'success', 'settlement', 'served'
...
RETURNS TRIGGER AS $$
BEGIN
    -- If status changes to COMPLETED/SELESAI, trigger stock deduction for all items
    -- V8: Removed 'paid', 'success', 'settlement', 'served'
    ...
    -- If status changes FROM completed to something else (cancelled/hold/paid), restore stock
    -- V8: Now moving from 'completed' to 'paid' will restore stock (because 'paid' is no longer a stock-out status)
