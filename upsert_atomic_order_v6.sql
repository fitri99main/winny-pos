-- SMART ATOMIC UPSERT (v6 - HIGH PERFORMANCE)
-- Optimasi: Menghilangkan 'FOR UPDATE' untuk mencegah antrean (database busy)
-- Perbaikan: Penanganan item yang lebih efisien
...
BEGIN
    ...
    -- 1. SERVER-SIDE NUMBERING (OPTIMIZED)
    -- Menggunakan UPDATE ... RETURNING untuk menghindari lock contention
    IF (l_order_no IS NULL OR l_order_no = '') AND p_target_sale_id IS NULL THEN
        UPDATE public.store_settings 
        SET invoice_last_number = invoice_last_number + 1 
        WHERE id = 1 
        RETURNING invoice_prefix, invoice_last_number INTO l_prefix, l_next_no;
    ...
    IF p_target_sale_id IS NOT NULL THEN
        ...
        IF FOUND THEN
            ...
            l_updated := TRUE;
            -- Penting: Kita hapus item lama, tapi trigger stok harus cerdas (diperbaiki di trigger)
            DELETE FROM public.sale_items WHERE sale_id = l_sale_id;
        END IF;
    END IF;

    -- 3. Insert new sale if not updated
