...
--
-- =============================================================
-- FIX CHECKOUT IDEMPOTENCY + LIGHTER WIFI VOUCHER FLOW
-- Tujuan:
-- 1. Mencegah transaksi dobel saat app timeout tapi server terlambat merespon.
-- 2. Memendekkan window lock invoice dengan menunda nomor final sampai akhir transaksi.
-- 3. Mengurangi beban trigger voucher WiFi dengan bulk assign tanpa delete ulang.
-- =============================================================
...
BEGIN
  ...
  -- Idempotency guard untuk transaksi baru dari client mobile
  ...
  -- Jika update order lama dan client tidak mengirim order_no, pakai order_no yang sudah ada
  ...
  -- Generate nomor final hanya jika memang perlu.
  -- Untuk transaksi Hold -> Paid, nomor HOLD akan diganti invoice final di akhir.
  ...
  -- Gunakan placeholder unik agar insert/update sales bisa jalan dulu
  -- tanpa menahan lock invoice counter selama proses sale_items & trigger lain.
  ...
  -- Nomor final dibuat belakangan agar antrean counter lebih singkat.
  IF v_requires_final_order_no THEN
    SELECT
