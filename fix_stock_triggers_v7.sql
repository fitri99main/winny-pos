-- ==========================================
-- FIX STOCK TRIGGERS (V7 - FORCED BIGINT)
-- Handles Stock only for COMPLETED sales
-- Forces sale_item_id to be BIGINT
-- ==========================================

-- 1. FORCE CORRECT COLUMN TYPE
DO $$ 
BEGIN 
    ...
    ELSE
        -- If it exists, force it to BIGINT (in case it was UUID from v5)
        ALTER TABLE stock_movements ALTER COLUMN sale_item_id TYPE BIGINT USING sale_item_id::text::bigint;
    END IF;
END $$;
...
BEGIN
    ...
    IF v_is_delete THEN
        ...
        IF v_sale_status NOT IN ('completed', 'selesai', 'paid', 'success', 'settlement', 'served') THEN
            RETURN OLD;
        END IF;
    END IF;
...
BEGIN
    ...
    IF (LOWER(NEW.status) IN ('completed', 'selesai', 'paid', 'success', 'settlement', 'served')) AND 
       (LOWER(COALESCE(OLD.status, 'hold')) NOT IN ('completed', 'selesai', 'paid', 'success', 'settlement', 'served')) THEN
    ...
    ELSIF (LOWER(OLD.status) IN ('completed', 'selesai', 'paid', 'success', 'settlement', 'served')) AND 
          (LOWER(NEW.status) NOT IN ('completed', 'selesai', 'paid', 'success', 'settlement', 'served')) THEN
