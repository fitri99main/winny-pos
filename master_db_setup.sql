...
-- 8. Restaurant Management (Tables)
...
INSERT INTO public.tables (number, capacity)
SELECT * FROM (VALUES 
  ('T1', 4), ('T2', 4), ('T3', 4), ('T4', 2), ('T5', 6),
  ('T6', 4), ('T7', 4), ('T8', 2), ('T9', 8), ('T10', 4)
) AS v(number, capacity)
...
BEGIN
    ...
    END IF;
     -- Branches
    IF NOT EXISTS (SELECT FROM pg_policies WHERE tablename = 'branches' AND policyname = 'Enable all access') THEN
        CREATE POLICY "Enable all access" ON public.branches FOR ALL USING (true) WITH CHECK (true);
    END IF;
     -- Departments
    ...
    END IF;
    -- Shifts
    IF NOT EXISTS (SELECT FROM pg_policies WHERE tablename = 'shifts' AND policyname = 'Enable all access') THEN
        CREATE POLICY "Enable all access" ON public.shifts FOR ALL USING (true) WITH CHECK (true);
    END IF;
    -- Shift Schedules
    IF NOT EXISTS (SELECT FROM pg_policies WHERE tablename = 'shift_schedules' AND policyname = 'Enable all access') THEN
        CREATE POLICY "Enable all access" ON public.shift_schedules FOR ALL USING (true) WITH CHECK (true);
    END IF;
    -- Accounts
    ...
    END IF;
    -- Roles
    IF NOT EXISTS (SELECT FROM pg_policies WHERE tablename = 'roles' AND policyname = 'Enable all access') THEN
        CREATE POLICY "Enable all access" ON public.roles FOR ALL USING (true) WITH CHECK (true);
    END IF;

    -- Tables
    IF NOT EXISTS (SELECT FROM pg_policies WHERE tablename = 'tables' AND policyname = 'Enable all access') THEN
        CREATE POLICY "Enable all access" ON public.tables FOR ALL USING (true) WITH CHECK (true);
    END IF;
