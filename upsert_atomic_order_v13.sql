-- Fungsi Upsert Sale with Items (v13 - SMART NUMBERING SYNC WITH WEB)
-- Mengikuti pengaturan store_settings (Mode Auto vs Manual)
...
BEGIN
  ...
  -- 2. Logika penomoran jika belum ada
  IF v_order_no IS NULL OR v_order_no = '' THEN
    IF LOWER(v_status) = 'pending' THEN
        -- Hold selalu menggunakan timestamp agar unik cepat
        v_order_no := 'HOLD-' || TO_CHAR(NOW(), 'YYMMDDHH24MISS');
    ELSE
        -- Ambil pengaturan dari store_settings (ID 1)
        SELECT invoice_mode, invoice_prefix, invoice_last_number 
        INTO v_inv_mode, v_inv_prefix, v_inv_last_no 
        ...
        ELSE
            -- Mode Manual (Prefix-Year-Timestamp)
