-- ==========================================
-- FIX STOCK TRIGGERS (DEDUCT & RESTORE)
-- ==========================================
    ...
    IF EXISTS (SELECT 1 FROM product_recipes WHERE product_id = NEW.product_id) THEN
        FOR r_recipe IN SELECT ingredient_id, amount FROM product_recipes WHERE product_id = NEW.product_id LOOP
            ...
            BEGIN
                ...
                INSERT INTO stock_movements (ingredient_id, ingredient_name, type, quantity, unit, reason, "user")
                VALUES (r_recipe.ingredient_id, v_ing_name, 'OUT', v_deduct_amount, v_ing_unit, 'Sold: ' || v_product_name || ' (' || NEW.quantity || ')', 'System');
            END;
        END LOOP;
    ELSE
        DECLARE
        ...
        BEGIN
            ...
            WHERE name = v_product_name OR code = v_product_name LIMIT 1;
            IF v_match_id IS NOT NULL THEN
                UPDATE ingredients SET current_stock = current_stock - NEW.quantity, last_updated = CURRENT_DATE WHERE id = v_match_id;
                INSERT INTO stock_movements (ingredient_id, ingredient_name, type, quantity, unit, reason, "user")
                VALUES (v_match_id, v_match_name, 'OUT', NEW.quantity, v_match_unit, 'Sold: ' || v_product_name || ' (' || NEW.quantity || ') [Auto]', 'System');
            ELSE
    ...
    IF EXISTS (SELECT 1 FROM product_recipes WHERE product_id = OLD.product_id) THEN
        FOR r_recipe IN SELECT ingredient_id, amount FROM product_recipes WHERE product_id = OLD.product_id LOOP
            ...
            BEGIN
                ...
                INSERT INTO stock_movements (ingredient_id, ingredient_name, type, quantity, unit, reason, "user")
                VALUES (r_recipe.ingredient_id, v_ing_name, 'IN', v_restore_amount, v_ing_unit, 'Restored: ' || v_product_name || ' (' || OLD.quantity || ')', 'System');
            END;
        END LOOP;
    ELSE
        DECLARE
        ...
        BEGIN
            ...
            IF v_match_id IS NOT NULL THEN
                ...
                INSERT INTO stock_movements (ingredient_id, ingredient_name, type, quantity, unit, reason, "user")
                VALUES (v_match_id, v_match_name, 'IN', OLD.quantity, v_match_unit, 'Restored: ' || v_product_name || ' (' || OLD.quantity || ') [Auto]', 'System');
            ELSE
