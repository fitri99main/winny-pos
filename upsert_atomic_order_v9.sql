-- Fungsi Upsert Sale with Items (v9 - MEGA ROBUST)
-- Menjamin nama produk tetap muncul dengan memeriksa berbagai kemungkinan label kunci JSON
...
BEGIN
  ...
  IF v_order_no IS NULL OR v_order_no = '' THEN
    v_order_no := 'TRX-' || TO_CHAR(NOW(), 'YYMMDDHH24MISS');
  END IF;

  -- 2. Upsert Tabel Sales
  ...
  -- Gunakan ID target jika RETURNING tidak memberikan hasil
  v_sale_id := COALESCE(v_sale_id, p_target_sale_id);

  -- 3. Kelola Items jika v_sale_id valid
  IF v_sale_id IS NOT NULL AND jsonb_array_length(p_items_data) > 0 THEN
    -- Hapus item lama (PENTING untuk update pesanan hold)
    DELETE FROM sale_items WHERE sale_id = v_sale_id;

    -- Iterasi manual setiap item untuk fleksibilitas label kunci
    FOR v_item_json IN SELECT * FROM jsonb_array_elements(p_items_data) LOOP
      -- Ambil data dengan fallback label (name vs product_name)
      ...
      -- Jika nama masih 'Produk' dan ada product_id, coba cari di tabel products
